...
In practice dish 1 is never confirmed at exactly `sessionStart` — there's always a real walk-to-the-oven delay. Real-device testing showed that leaving `plannedOffset` anchored to `sessionStart` regardless silently breaks the "finish together" property by however long that delay was: dish 2's `readyToAdd` alert would fire that much *sooner*, relative to dish 1 actually going in, than intended. So the **first confirmation in a session** — which is always one of the offset-0 dishes, since no other dish can be `readyToAdd` yet — re-anchors every still-`pending` dish: each one's `plannedAddAt` shifts forward by exactly the delay the anchor dish incurred (`confirmedAt − sessionStart`). This is a one-time shift: only the first anchor confirmation triggers it, so a second dish sharing that same offset-0 cohort doesn't shift things again. Dishes already `readyToAdd` by that point are **not** shifted — their alert already fired, and the late-add rule already isolates them individually.
...
`doneAt` always derives from the dish's **own `confirmedAt`**, never from the planned offset. This single rule is the late-add handling: a late confirmation shifts only that dish's finish later; `cooking` dishes run independently and are never touched by another dish's confirmation. `plannedOffset` values for still-`pending` dishes are fixed at session start with exactly one exception — the anchor-confirmation shift described in §3.1 — and are otherwise never recalculated. A dish left in `readyToAdd` waits indefinitely — no timeout, no auto-confirm.
...
Critical alerts, both paths, **add alerts only, opt-in via the critical-alerts toggle**: iOS `push.interruption-level: critical` + `sound: {critical: 1, volume: ...}` (plays even with Do Not Disturb on or the phone muted — a genuine Apple Critical Alert, not just time-sensitive); Android `priority: high` + `channel: alarm_stream` + `ttl: 0`.

`steamtime_dish_done` and `steamtime_session_completed` alerts, both paths, are **always** sent `priority: high` (Android) + `push.interruption-level: time-sensitive` (iOS), unconditionally — not gated by the critical-alerts toggle. A finished dish deserves prompt delivery by default (it can overcook or go cold while a notification sits throttled), but this is deliberately one notch below the opt-in critical tier: `time-sensitive`/`priority: high` ensures fast delivery without bypassing Do Not Disturb the way the critical toggle does for add alerts.
...
**Entity ids are not stable strings to hardcode — a real bug found on first real-device test.** The card originally assumed fixed ids (`sensor.steamtime_session`, etc.). Home Assistant prepends a device's area slug to every one of its entities' ids once the device is assigned to an area (e.g. `sensor.keuken_steamtime_dish_library`), and a user can rename any entity at any time regardless — so a shipped card can never assume its own integration's entity ids verbatim. Fixed by matching on the stable *object-id suffix* instead (`findEntity(hass, "steamtime_dish_library")` scans `hass.states` for any `sensor.*` key ending in that suffix), which survives both area assignment and renaming. Worth remembering for any future entity id assumptions anywhere in the card.
...
*v1.6 (post-launch, real-device bugfix) — §12: the card no longer hardcodes entity ids. First real-device test (a device assigned to an area) showed Home Assistant prepends the area slug to every entity id of that device (`sensor.keuken_steamtime_dish_library`, not `sensor.steamtime_dish_library`), which a fixed-string assumption can never account for — the same is true for any user rename, unrelated to areas. The card now finds its sensors by matching the stable object-id suffix against `hass.states` instead. No backend change; `www/steamtime-card.js` only.*
